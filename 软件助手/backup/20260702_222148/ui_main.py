...
from widgets import EyeToggleButton, MoistureCurveWidget, PillButton, RadarWidget, SoilMoistureBar


def card_style(radius: int = 22) -> str:
    return f"background: #ffffff; border: 1px solid #edf2ee; border-radius: {radius}px;"
...
class LoginPage(QWidget):
    ...
    def _build_ui(self) -> None:
        self.setStyleSheet("background: #eef8f1;")
        ...
        headline.setFont(QFont("Microsoft YaHei UI Light", 34))
        headline.setStyleSheet("color: #0e2e22; letter-spacing: 0px;")
        root.addWidget(headline)

        subtitle = QLabel("植物智能助手 · 环境感知与养护控制中枢")
        subtitle.setAlignment(Qt.AlignCenter)
        subtitle.setFont(QFont("Microsoft YaHei UI", 13))
        subtitle.setStyleSheet("color: #65866f;")
        root.addWidget(subtitle)
        root.addSpacing(42)

        card = QFrame()
        card.setFixedWidth(580)
        card.setStyleSheet(card_style(28))
        card_layout = QVBoxLayout(card)
        card_layout.setContentsMargins(40, 28, 40, 34)
        card_layout.setSpacing(14)

        title = QLabel("唤醒系统登录")
        title.setAlignment(Qt.AlignCenter)
        title.setFont(QFont("Microsoft YaHei UI", 18, QFont.Bold))
        title.setStyleSheet("color: #0d281d;")
        card_layout.addWidget(title)

        self.phone_edit = QLineEdit()
        self.phone_edit.setPlaceholderText("请输入手机号")
        self.phone_edit.setMinimumHeight(46)
        ...
        self.tip_label.setStyleSheet("color: #d45c57; font-size: 10.5pt;")
...
class WakePage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        headline.setStyleSheet("color: #0e2e22;")
        root.addWidget(headline)
        ...
        frame = QFrame()
        frame.setFixedSize(460, 250)
        frame.setStyleSheet(card_style(28))
        frame_layout = QVBoxLayout(frame)
        frame_layout.setContentsMargins(28, 24, 28, 24)
        frame_layout.addStretch(1)
        self.wake_button = PillButton("唤醒系统")
        self.wake_button.setMinimumHeight(60)
        self.wake_button.setMaximumWidth(320)
        self.wake_button.clicked.connect(self.wake_requested.emit)
        holder = QHBoxLayout()
        holder.addStretch(1)
        holder.addWidget(self.wake_button)
        holder.addStretch(1)
        frame_layout.addLayout(holder)
        frame_layout.addStretch(1)

        center = QHBoxLayout()
        center.addStretch(1)
        center.addWidget(frame)
        center.addStretch(1)
...
class DashboardPage(QWidget):
    ...
    def _build_ui(self) -> None:
        self.setStyleSheet("background: #f4f8f5;")
        root = QVBoxLayout(self)
        ...
        self.time_banner.setMinimumSize(760, 90)
        ...
        profile = QFrame()
        profile.setFixedSize(260, 76)
        profile.setStyleSheet("background: #ffffff; border: 1px solid #d7e7de; border-radius: 20px;")
        profile_layout = QHBoxLayout(profile)
        profile_layout.setContentsMargins(14, 10, 14, 10)
        avatar = QLabel("植")
        avatar.setAlignment(Qt.AlignCenter)
        avatar.setFixedSize(40, 40)
        avatar.setStyleSheet("background: #eef5f1; border-radius: 20px; font-size: 20px;")
        profile_layout.addWidget(avatar)

        profile_text = QVBoxLayout()
        profile_text.setSpacing(2)
        self.user_id_label = QLabel("")
        self.user_id_label.setFont(QFont("Microsoft YaHei UI", 10, QFont.Bold))
        self.user_state_label = QLabel("已登录")
        self.user_state_label.setStyleSheet("color: #35be63; font-size: 10pt;")
        self.logout_label = QLabel('<a href="logout" style="color:#f05c47;text-decoration:none;">退出登录</a>')
        ...
        body.addWidget(self._build_inspection_card(), 0, 0)
        self.serial_matrix = SerialMatrixPanel(send_callback=self._handle_send_command)
        self.serial_matrix.setStyleSheet(card_style(24))
        body.addWidget(self.serial_matrix, 0, 1)

        body.addWidget(self._build_radar_card(), 1, 0)
        self.ai_panel = AiCommandPanel(send_callback=self._handle_send_command)
        self.ai_panel.setStyleSheet(card_style(24))
        body.addWidget(self.ai_panel, 1, 1)

        body.addWidget(self._build_curve_card(), 2, 0)
        spacer = QWidget()
        body.addWidget(spacer, 2, 1)
        root.addLayout(body)

    def _build_card_title(self, text: str) -> QLabel:
        label = QLabel(text)
        label.setFont(QFont("Microsoft YaHei UI", 16, QFont.Bold))
        label.setStyleSheet("color: #0a2d1d;")
        return label

    def _build_inspection_card(self) -> QFrame:
        card = QFrame()
        card.setStyleSheet(card_style(24))
        ...
        self.plant_status_label.setStyleSheet("color: #9fe4b4;")
        ...
        self.inspection_time_label.setStyleSheet("color: #9fe4b4;")
    ...
    def _metric_box(self, title: str, value: str) -> QFrame:
        box = QFrame()
        box.setStyleSheet("background: #f5faf7; border-radius: 16px;")
        ...
        value_label.setStyleSheet("color: #0b3323;")
    ...
    def _build_curve_card(self) -> QFrame:
        ...
        self.curve_current_label.setStyleSheet("color: #0b3323;")
    ...
    def _refresh_dashboard(self) -> None:
        ...
        self.curve_widget.set_curve_data([42.0, 42.0, 42.0, 42.0, 42.0, 42.0], labels)
