...
class MoistureCurveWidget(QWidget):
    ...
    def paintEvent(self, event) -> None:
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)
        rect = self.rect().adjusted(22, 18, -24, -34)
        chart_rect = QRectF(rect.left() + 42, rect.top() + 10, rect.width() - 50, rect.height() - 36)
        ...
        painter.drawText(QRectF(rect.left(), chart_rect.bottom() + 2, 90, 24), "土壤湿度")
        painter.drawText(QRectF(chart_rect.center().x() - 40, chart_rect.bottom() + 2, 80, 24), "时间轴")
        ...
        painter.drawText(QRectF(chart_rect.left() - 8, chart_rect.bottom() + 14, 80, 20), labels[0])
        painter.drawText(QRectF(chart_rect.center().x() - 35, chart_rect.bottom() + 14, 80, 20), labels[1])
        painter.drawText(QRectF(chart_rect.right() - 66, chart_rect.bottom() + 14, 80, 20), labels[2])
