...
class ToggleActionButton(PushButton := QPushButton):
    ...
    def _sync_style(self, checked: bool) -> None:
        self.apply_visual_state(checked, self.active_text if checked else self.base_text)

    def apply_visual_state(self, highlighted: bool, text: Optional[str] = None) -> None:
        if text is not None:
            self.setText(text)
        bg = "#dff1e8" if highlighted else "#f3f8f4"
        border = "#35be63" if highlighted else "#d9e8de"
        fg = "#2c8450" if highlighted else "#37684a"
        self.setStyleSheet(
            f"""
            QPushButton {{
                background: {bg};
            ...
            QPushButton:hover {{
                background: {'#d5eddf' if highlighted else '#ebf4ee'};
            }}
            """
        )
