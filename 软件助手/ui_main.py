...
class DashboardPage(QWidget):
    ...
    def __init__(self, auth_manager: AuthStateManager, parent=None) -> None:
        ...
        self.current_leaf_status = "未识别到植物"
        self.current_plant_type = "常规"
        self.current_plant_type_code = 0x02
        self._pump_enabled = False
        self._pump_manual_override = False
        self.moisture_history = [0.0] * 6
        self.moisture_time_labels = ["--:--:--"] * 3
        self._received_sensor_types = set()
        self._build_ui()
        self._start_timers()
    ...
    def _build_ui(self) -> None:
        ...
        self.serial_matrix.sensor_data_received.connect(self._handle_sensor_packet)
        self.serial_matrix.plant_type_changed.connect(self._handle_plant_type_changed)
        self.serial_matrix.pump_manual_toggled.connect(self._handle_manual_pump_changed)
        ...
        self.ai_panel.setMinimumHeight(340)
    ...
    def _start_timers(self) -> None:
        ...
        self.timestamp_timer.timeout.connect(self._send_timestamp_if_connected)
        self.timestamp_timer.start(1000)

        self.pump_manual_override_timer = QTimer(self)
        self.pump_manual_override_timer.setSingleShot(True)
        self.pump_manual_override_timer.timeout.connect(self._clear_manual_pump_override)

    def start_interaction_session(self) -> None:
        self.system_started_at = datetime.now()
        self.serial_matrix.set_auto_seek_state(False, send=False)

    def _send_timestamp_if_connected(self) -> None:
        if self.serial_matrix.is_connected():
            self.serial_matrix.send_timestamp()
    ...
    def _handle_send_command(self, payload: str) -> None:
        sent = self.serial_matrix.send_payload(payload)
        self._sync_actuator_state_from_payload(payload)
        self.ai_panel.append_report(self._command_text(payload, sent))
        if not sent:
            self.ai_panel.append_report("串口：未连接，指令未发送。")

    def _sync_actuator_state_from_payload(self, payload: str) -> None:
        try:
            data = bytes.fromhex(payload.strip())
        except ValueError:
            return
        if len(data) != 4 or data[0] != 0xAA or data[3] != 0x55:
            return
        if data[1] == 0x20:
            self._pump_enabled = data[2] == 0x01

    def _command_text(self, payload: str, sent: bool) -> str:
        try:
            data = bytes.fromhex(payload.strip())
        except ValueError:
            return f"发送文字指令：{payload}" if sent else f"文字指令未发送：{payload}"
        suffix = "" if sent else "（未发送）"
        if len(data) == 4 and data[0] == 0xAA and data[3] == 0x55:
            command, value = data[1], data[2]
            if command == 0x10:
                action = "开启" if value == 0x02 else "关闭"
                return f"自动寻光：{action}{suffix}"
            if command == 0x20:
                action = "开启" if value == 0x01 else "关闭"
                return f"水泵：{action}{suffix}"
            if command == 0x30:
                action = "开启" if value == 0x01 else "关闭"
                return f"补光：{action}{suffix}"
            if command == 0x40:
                action = "开启" if value == 0x01 else "关闭"
                return f"急停：{action}{suffix}"
            if command == 0x70:
                plant_type = {0x01: "耐旱", 0x02: "常规", 0x03: "喜湿/水培"}.get(value, "未知")
                return f"植物类型：切换为{plant_type}{suffix}"
        return "控制指令已发送" if sent else "控制指令未发送"

    def _handle_sensor_packet(self, packet: dict) -> None:
        packet_type = packet.get("type")

        if packet_type == "soil_moisture":
            value = self._keep_last_valid_float(
                packet.get("value"), self.current_moisture, packet_type, allow_zero=True
            )
            self.current_moisture = value
            self._push_moisture_history(value)
        elif packet_type == "temperature":
            self.current_temperature = self._keep_last_valid_float(
                packet.get("value"), self.current_temperature, packet_type
            )
        elif packet_type == "air_humidity":
            self.current_humidity = self._keep_last_valid_float(packet.get("value"), self.current_humidity, packet_type)
        elif packet_type == "air_quality":
            self.current_air_ppm = self._keep_last_valid_int(packet.get("value"), self.current_air_ppm, packet_type)
        elif packet_type == "obstacle":
            self.current_obstacle_angle = packet.get("angle")
            self.current_obstacle_distance = self._keep_last_valid_float(
                packet.get("distance"), self.current_obstacle_distance, packet_type, allow_zero=False
            )
        elif packet_type == "leaf_status":
            self.current_leaf_status = packet.get("text") or self.current_leaf_status
        elif packet_type == "plant_type":
            self.current_plant_type = packet.get("text") or self.current_plant_type
            self.current_plant_type_code = packet.get("code") or self.current_plant_type_code
            self._sync_plant_type_ui(self.current_plant_type_code)

        self._update_pump_automation()
        self._refresh_dashboard()

    def _handle_plant_type_changed(self, code: int, text: str) -> None:
        self.current_plant_type_code = code
        self.current_plant_type = text
        self.ai_panel.append_report(f"植物类型已同步为：{text}")
        self._update_pump_automation(force_log=True)
        self._refresh_dashboard()

    def _handle_manual_pump_changed(self, enabled: bool) -> None:
        self._pump_manual_override = True
        self._pump_enabled = enabled
        self.pump_manual_override_timer.start(self.PUMP_MANUAL_OVERRIDE_MS)
        action = "开启" if enabled else "关闭"
        seconds = self.PUMP_MANUAL_OVERRIDE_MS // 1000
        self.ai_panel.append_report(f"手动水泵：已{action}，{seconds} 秒后恢复自动阈值控制。")

    def _clear_manual_pump_override(self) -> None:
        if not self._pump_manual_override:
            return
        self._pump_manual_override = False
        self.ai_panel.append_report("手动水泵：临时接管结束，已恢复自动阈值控制。")
        self._update_pump_automation(force_log=True)

    def _moisture_threshold(self) -> float:
        return {
            0x01: 5.0,
            0x02: 25.0,
            0x03: 40.0,
        }.get(self.current_plant_type_code, 25.0)

    def _update_pump_automation(self, force_log: bool = False) -> None:
        if self._pump_manual_override:
            if force_log:
                state = "开启" if self._pump_enabled else "关闭"
                self.ai_panel.append_report(f"手动水泵：当前保持{state}，自动阈值控制暂停。")
            return
        if "soil_moisture" not in self._received_sensor_types:
            if force_log:
                self.ai_panel.append_report("自动水泵：等待土壤湿度数据后开始阈值控制。")
            return
        threshold = self._moisture_threshold()
        should_enable = self.current_moisture < threshold
        if should_enable == self._pump_enabled:
            if force_log:
                state = "开启" if self._pump_enabled else "关闭"
                self.ai_panel.append_report(f"自动水泵：{self.current_plant_type}阈值 {threshold:.0f}%，当前保持{state}。")
            return

        self._pump_enabled = should_enable
        self.serial_matrix.set_pump_state(should_enable, send=True)
        action = "开启" if should_enable else "关闭"
        relation = "低于" if should_enable else "高于或等于"
        self.ai_panel.append_report(
            f"自动水泵：土壤湿度 {self.current_moisture:.1f}% {relation} {threshold:.0f}% 阈值，已{action}水泵。"
        )
    ...
    def _refresh_dashboard(self) -> None:
        ...
        self.plant_status_label.setText(f"当前植物状态：{self.current_leaf_status}")

        self.moisture_bar.setValue(self.current_moisture)
        self.moisture_value_label.setText(f"{self.current_moisture:.1f}%")
        self.soil_title_label.setText(f"土壤湿度（植物类型：{self.current_plant_type}）")
        ...
        self.curve_widget.set_curve_data(self.moisture_history, self.moisture_time_labels)
    ...
    def _keep_last_valid_float(self, value, last_value, sensor_type: Optional[str] = None, allow_zero: bool = False) -> float:
        if value is None:
            return last_value
        try:
            numeric = float(value)
            ...
            return last_value
        has_received_real_value = sensor_type in self._received_sensor_types
        if not allow_zero and has_received_real_value and numeric == 0 and last_value not in (None, 0):
            return last_value
        if sensor_type:
            self._received_sensor_types.add(sensor_type)
        return numeric

    def _keep_last_valid_int(self, value, last_value, sensor_type: Optional[str] = None, allow_zero: bool = False) -> int:
        numeric = self._keep_last_valid_float(value, last_value, sensor_type=sensor_type, allow_zero=allow_zero)
        return int(round(numeric))

    def _push_moisture_history(self, value: float) -> None:
        self.moisture_history.append(value)
        self.moisture_history = self.moisture_history[-6:]
        timestamp = datetime.now().strftime("%H:%M:%S")
        if not self.moisture_time_labels or self.moisture_time_labels[-1] != timestamp:
            self.moisture_time_labels.append(timestamp)
            self.moisture_time_labels = self.moisture_time_labels[-3:]

    def _sync_plant_type_ui(self, code: Optional[int]) -> None:
        if code is None:
            return
        combo_index = {0x01: 0, 0x02: 1, 0x03: 2}.get(code)
        if combo_index is None:
            return
        self.serial_matrix.guard_combo.blockSignals(True)
        self.serial_matrix.guard_combo.setCurrentIndex(combo_index)
        self.serial_matrix.guard_combo.blockSignals(False)
...
class PlantAssistantMainWindow(QMainWindow):
    ...
    def _show_dashboard_page(self) -> None:
        self.dashboard_page.serial_matrix.start_port_monitor()
        self.dashboard_page.start_interaction_session()
        self.dashboard_page._refresh_dashboard()
